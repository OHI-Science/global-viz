...
\title{\textbf{\textsc{NAME OF REGION HERE}  - \hspace{8pt}Ocean Health Index Project Region Summary Card}}
\institute{National Center for Ecological Analysis and Synthesis}
\titlegraphic{\includegraphics[width=0.6\linewidth]{example-image}}
...
{%
\begin{tikzfigure}[Scores over time using the 2018 methods.]
